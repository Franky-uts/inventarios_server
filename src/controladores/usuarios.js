...
export const getUsuarios = async (req,res)=>{
    const {rows} = await pool.query(`Select "Nombre", "Puesto" from public."Usuarios";`)
    res.send(rows)
}
...
export const getUsuario = async (req,res)=>{
    ...
    const {rows} = await pool.query(`Select "Nombre", "Puesto" from public."Usuarios" WHERE "Nombre" = '${usuario}';`)
    if(rows.length>0){
        const {rows} = await pool.query(`Select "Nombre", "Puesto" from public."Usuarios" WHERE "Nombre" = '${usuario}' AND "Contraseña" = '${contr}';`)
        if(rows.length>0){
            res.send(rows)
        } else{
...
export const editarUsuario = async (req,res)=>{
    ...
    if(columna != "Nombre"){
        ...
        res.status(409).send('El nombre de usuario no se puede modificar')
    }
}